            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">{{ section.title }} 생성 진행 중</span>
                </div>
              </div>
            </div>
          </div>
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const entityMatch = text.match(/관련 엔터티:\s*(\d+)/)
    const relMatch = text.match(/관계 체인:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])

    const subQSection = text.match(/### 분석 하위 질문\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    const entitySection = text.match(/### \[핵심 엔터티\]\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const entitySection = text.match(/### \[관련 엔터티\]\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*선정([^(]+?)(?:\(index\s*=?\s*\d+\))?[::]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        } else if (currentName && line.trim() && !line.match(/^미선정|^종합|^최종 선정/)) {
    ...
    interviewBlocks.forEach(block => {
      ...
      const nameRoleMatch = block.match(/\*\*Name:\*\*\s*(.+?)\s*\|\s*\*\*Role:\*\*\s*(.+?)(?:\n|$)/)
      if (nameRoleMatch) {
        interview.name = nameRoleMatch[1].trim()
        interview.role = nameRoleMatch[2].trim()
        if (individualReasons[interview.name]) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    const queryMatch = text.match(/검색 질의:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()

    const countMatch = text.match(/찾은\s*(\d+)\s*건/)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `핵심 엔터티 (${props.result.entities.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '시계열 기억에 연결된 최신 핵심 사실'),
          ...
          }, expandedFacts.value ? `접기 ▲` : `전체 ${props.result.facts.length}건 ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '핵심 엔터티'),
          ...
          }, expandedEntities.value ? `접기 ▲` : `전체 ${props.result.entities.length}개 ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          ...
          }, expandedRelations.value ? `접기 ▲` : `전체 ${props.result.relations.length}건 ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '드리프트 쿼리 기반 분석 하위 질문'),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, '핵심 엔터티가 없습니다'),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `관련 엔터티 (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          }, expandedHistorical.value ? `접기 ▲` : `전체 ${props.result.historicalFacts.length}건 ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '관련 엔터티'),
            h('span', { class: 'panel-count' }, `총 ${props.result.entities.length}개`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, '관련 엔터티가 없습니다'),
