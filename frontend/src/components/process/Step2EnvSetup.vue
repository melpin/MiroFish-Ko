    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            simulation 인스턴스를 생성하고 시뮬레이션 월드 기본 파라미터 템플릿을 불러옵니다.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            컨텍스트를 바탕으로 도구를 자동 호출해 지식 그래프에서 엔티티/관계를 정리하고, 시뮬레이션 개체를 초기화한 뒤 현실 시드를 기반으로 행동과 기억을 부여합니다.
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">현실 시드 연관 주제 수</span>
            </div>
          </div>
            ...
            <div class="profiles-list">
              ...
              >
                ...
                <div class="profile-meta">
                  <span class="profile-profession">{{ profile.profession || '직업 정보 없음' }}</span>
                </div>
      ...
      <!-- Step 03: 양대 플랫폼 시뮬레이션 설정 생성 -->
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">03</span>
            <span class="step-title">양대 플랫폼 시뮬레이션 설정 생성</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM이 시뮬레이션 요구사항과 현실 시드를 바탕으로 세계 시간 흐름, 추천 알고리즘, 개체별 활동 시간대, 발화 빈도, 이벤트 트리거 등을 자동 설정합니다.
          </p>
              ...
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  ...
                  <div class="platform-params">
                    ...
                    <div class="param-row">
                      <span class="param-label">확산 임계치</span>
      ...
      <!-- Step 04: 초기 활성 시나리오 편성 -->
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">04</span>
            <span class="step-title">초기 활성 시나리오 편성</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            내러티브 방향에 따라 초기 활성 이벤트와 핵심 토픽을 자동 생성해 시뮬레이션 세계의 시작 상태를 구성합니다.
          </p>
          ...
          <div v-if="simulationConfig?.event_config" class="orchestration-content">
            ...
            <div class="initial-posts-section">
              <span class="box-label">초기 활성 시퀀스 ({{ simulationConfig.event_config.initial_posts.length }})</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">시뮬레이션 환경 준비가 완료되었습니다. 이제 시뮬레이션을 시작할 수 있습니다.</p>
          
          <!-- 시뮬레이션 라운드 설정: 구성 생성 후 라운드 계산이 완료되면 표시 -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">시뮬레이션 라운드 설정</span>
                <span class="section-desc">MiroFish가 현실 시간 <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span>시간을 자동 계획합니다. 라운드 1회는 현실 시간 <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span>분에 해당합니다.</span>
              </div>
              <label class="switch-control">
                <input type="checkbox" v-model="useCustomRounds">
                <span class="switch-track"></span>
                <span class="switch-label">직접 설정</span>
              </label>
            </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>Agent 규모가 100명일 때 예상 소요 시간: 약 {{ Math.round(customMaxRounds * 0.6) }}분</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        Agent 규모가 100명일 때 예상 소요 시간: {{ Math.round(autoGeneratedRounds * 0.6) }}분
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">첫 실행이라면 라운드 수를 줄일 수 있는 '직접 설정' 모드 전환을 권장합니다. 더 빠르게 결과를 확인하고 오류 위험을 낮출 수 있습니다. ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">표면 연령</span>
              <span class="info-value">{{ selectedProfile.age || '-' }}세</span>
            </div>
            <div class="info-item">
              <span class="info-label">표면 성별</span>
            ...
            <div class="info-item">
              <span class="info-label">표면 MBTI</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">사건 전개 경험</span>
                <span class="dim-desc">해당 사건에서의 행동 흐름</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">행동 패턴 프로파일</span>
                <span class="dim-desc">경험 기반 행동 성향</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">고유 기억 흔적</span>
                <span class="dim-desc">현실 시드로 형성된 기억</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">사회 관계 네트워크</span>
                <span class="dim-desc">개체 간 연결과 상호작용 구조</span>
              </div>
            </div>
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // 설정 생성 단계 진입 후 설정 폴링 시작
    if (!configTimer) {
      addLog('양대 플랫폼 시뮬레이션 설정 생성을 시작합니다...')
      startConfigPolling()
    }
  } else if (newStage === 'copying_scripts') {
    phase.value = 2 // 계속 설정 단계
  }
})

// 설정에서 자동 라운드 계산(하드코딩 기본값 미사용)
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // 설정이 아직 생성되지 않음
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // 설정 데이터가 불완전함
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // 슬라이더 범위 이상 방지를 위해 최대 라운드 최솟값을 40(권장값)으로 보정
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // 사용자 지정 라운드 사용 시 max_rounds 전달
    params.maxRounds = customMaxRounds.value
    addLog(`시뮬레이션 시작(사용자 지정): ${customMaxRounds.value}회`)
  } else {
    // 자동 생성 라운드 유지 시 max_rounds 미전달
    addLog(`시뮬레이션 시작(자동 설정): ${autoGeneratedRounds.value}회`)
  }
...
const startPrepareSimulation = async () => {
  ...
  // 1단계 완료 처리 후 2단계 시작
  phase.value = 1
  addLog(`시뮬레이션 인스턴스 생성 완료: ${props.simulationId}`)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('이미 완료된 준비 작업이 있어 그대로 사용합니다.')
      ...
      // 예상 Agent 총수 즉시 반영(prepare 응답 사용)
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(`Zep 그래프에서 ${res.data.expected_entities_count}개 엔티티를 확인했습니다.`)
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(`  └─ 엔티티 유형: ${res.data.entity_types.join(', ')}`)
        }
      }
      
      addLog('준비 진행률 폴링을 시작합니다...')
      // 진행률 폴링 시작
      startPolling()
      // Profiles 실시간 조회 시작
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // API 유효값이 있을 때만 갱신(기존 유효값 덮어쓰기 방지)
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        addLog(`→ Agent 페르소나 ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || '직업 정보 없음'})`)
        
        // 전원 생성 완료 시
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ 총 ${currentCount}명의 Agent 페르소나 생성 완료`)
        }
      }
    }
  } catch (err) {
    console.warn('Profiles 조회 실패:', err)
  }
}
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          ...
          addLog('LLM으로 시뮬레이션 설정 파라미터 생성 중...')
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.config.time_config) {
          ...
          addLog(`시간 설정: 라운드당 ${tc.minutes_per_round}분, 총 ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}회`)
        }
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ 환경 구성이 완료되어 시뮬레이션을 시작할 수 있습니다.')
        emit('update-status', 'completed')
      }
    }
  } catch (err) {
    console.warn('Config 조회 실패:', err)
  }
}

const loadPreparedData = async () => {
  phase.value = 2
  addLog('기존 설정 데이터를 불러오는 중...')

  // 마지막으로 Profiles 1회 조회
  await fetchProfilesRealtime()
  addLog(`${profiles.value.length}명의 Agent 페르소나를 불러왔습니다.`)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('✓ 시뮬레이션 설정 로드 완료')
        
        // 상세 설정 요약 출력
        if (res.data.summary) {
          addLog(`  ├─ Agent 수: ${res.data.summary.total_agents}명`)
